...
class ScrollGallery {
	 ...
	 private $nggsgversion = "1.8.1"; //Version
	 ...
	 function nggScrollGalleryReplace($content) {//old version
	  ...
	  function nggSGShow($sgconfig, $pictures = null, $buildThumbs = true, $buildImages = true) {	
		  ...
		  if ($galleryID) {
			...
			$galleryID = rand();//falls pictures als parameter �bergeben werden
		  }
...
/*Kyle trying to word wrap with PHP-src:http://www.php.net/manual/en/function.wordwrap.php*/
$balloon_caption = stripslashes($picture["desc"]);
$balloon_wrap = wordwrap($balloon_caption, 80, "<br />\n");
								 if($useCaptions==true&&$useDesc==true&&stripslashes($picture["desc"])!="")
														$outI .='<div>'.$balloon_wrap.'</div>';
												}
	 ...
	 function nggSGRegisterPluginLinks($links, $file) {
		...
		if ($file == $plugin) {
			return array_merge(
				$links,
				array( sprintf( '<a href="admin.php?page=%s">%s</a>', plugin_basename( dirname(__FILE__)), __('Settings') ), '<g:plusone size="small"  href="http://software.bmo-design.de/scrollgallery/wordpress-plugin-nextgen-scroll-gallery.html"></g:plusone>' )
			);
		}
		return $links;
	 }
