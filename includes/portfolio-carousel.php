 ...
 */
 ...
 if ( is_array( $entries ) && ! is_wp_error( $entries ) && count( $entries ) > 0 ) {
 	...
 	$count = 0;
 	$turnips = $woo_options['woo_portfolio_title'];
	$bananas = $woo_options['woo_portfolio_description'];
	$peeled = stripslashes($bananas);
	$html .= '<div class="turnips">' . $peeled . '</div>';
 	...
 	$html .= '<ul class="slides">' . "\n";
 		...
 		foreach ( $entries as $post ) {
 			...
 			$cat_title = '<div class="beans"><a href="' . $link . '">' . get_the_title() . '</a></div>';
 			...
 			$html .= '<li id="' . $carousel_id . '-item-number-' . $count . '" class="item-number-' . $count . ' item-id-' . get_the_ID() . '">' . $cat_title . $item_link . '</li>' . "\n";
 		}
